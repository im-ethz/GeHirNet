...
Paths inside historical artifacts may be Windows or Colab paths. Use the new `gehirnet` package for portable inference/training and inspect [reproducibility notes](../docs/REPRODUCIBILITY.md) before interpreting historical results. Run historical Python scripts from the repository root so their relative data paths continue to resolve. Notebook paths still require the original environment or manual configuration.
